...
// CalculateArrowDamage calculates arrow damage using Bedrock Edition's formula
// (PocketMine / BDS). Arrows add 97% of their velocity to their base damage.
// Critical arrows deal 9-10 damage before Power is applied.
...
// UseArrowDamage configures conf to use Bedrock arrow damage calculation with
// the given Power enchantment level. Custom arrow factories (trails, etc.)
// should call this when building a ProjectileBehaviourConfig.
func (conf *ProjectileBehaviourConfig) UseArrowDamage(powerLevel int) {
	conf.damageCalculator = CalculateArrowDamage
	conf.powerLevel = powerLevel
}
